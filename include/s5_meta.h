...
{
    int64 volume;                           ///< volume quota owned by tenant
    int64 iops;                             ///< iops quota of tenant
    int64 bw;                               ///< bandwidth quota of tenant
...
typedef struct s5_tray_module_info
{
	...
	int64  tray_raw_capacity;		    ///< tray raw capacity, unit: MB
	int64  tray_usable_capacity;		///< tray usable capacity, unit: MB
...
typedef struct s5_store_detailed_info
{
	...
	uint64 capacity_total;
	uint64 capacity_available;
...
typedef struct s5_quotaset
{
    ...
    int64 iops;							///< iops quota of quotaset
    int64 bw;							///< bandwidth quota of quotaset
} s5_quotaset_t;
...
typedef struct s5_client_link
{
	...
	uint64				rate_1;							///< iops quota of first level CAR
	int					cid_2;							///< the second level CAR ID
	uint64				rate_2;							///< iops quota of the second level CAR
	int					cid_3;							///< quota level CAR ID
	uint64				rate_3;							///< iops quota of the second level CAR
}s5_client_link_t;
...
typedef struct s5d_parent_info
{
	...
	uint64 snap_seq;		///< sequence number of snapshot
	uint64 overlap;		    ///< overlap size between parent and child volume
} s5d_parent_info_t;
...
{
	uint64 volume_id;
	uint64 replica_id[MAX_REPLICA_NUM];
	uint64 volume_size;		    ///< volume size in bytes
	uint64 features;
	int32 snap_seq;			    ///< snapContext-last sequence.
	int32 replica_count;
...
typedef struct s5_cltreq_volume_open_param
{
	...
	uint64 replica_id;                     // ALL_REPLICA opens all replicas, replica id opens the designated replica only. id is not index.
...
{
	char tenant_name[MAX_NAME_LEN];				///< tenant name
	uint32 start;								///< position of volume in result list, start from 0.
	uint32 count;								///< the selected number of volumes in result list.
...
{
	int64 volume;								///< volume quota of tenant to create
	int64 iops;									///< iops quota of tenant to create
	int64 cbs;									///< Committed Burst Size(CBS) quota for new tenant
	int64 bw;									///< band-width quota of tenant to create
...
typedef struct s5_cltreq_tenant_update_param
{
	...
	int64 volume;								///< new volume quota of tenant
	int64 iops;									///< new iops quota of tenant
	int64 cbs;									///< new cbs of tenant
	int64 bw;									///< new bandwidth quota of tenant
...
typedef struct s5_cltreq_volume_resize_param
{
	...
	uint64 resize;								///< new volume quota of volume
...
{
	uint32 start;								///< position of tenants in result list, start from 0.
	uint32 count;								///< the selected number of tenants in result list.
...
{
	uint32 start;								///< position of volume in result list, start from 0.
	uint32 count;								///< expected number of volume in result list
